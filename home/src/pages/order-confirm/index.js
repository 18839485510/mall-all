...
var page = {
    ...
    renderShipping(shippings) {
        ...
        var html = utils.render(shippingTpl, {
            shippings: shippings
        })
    ...
    loadProductList: function () {
        ...
        api.getOrdersProducts({
            success: function (result) {
                if (result.data.cartList.length > 0) {
                    var html = utils.render(productTpl, result.data)
    ...
    bindEvent: function () {
        ...
        this.$shippingBox.on('click', '.shipping-add', function () {
            ...
            if (_this.$shippingBox.find('.shipping-item').length > 4) {
                utils.showErrorMsg('您的地址数已经达到上限5个了')
            } else {
                _modal.show()
            }
        })
